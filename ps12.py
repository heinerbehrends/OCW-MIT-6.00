...
##        self.isVirusResistant = False
        for virus in self.viruses:
            self.isVirusResistent = False
            for drug in drugResist:
                if virus.getResistance(drug):
                    self.isVirusResistant = True
                else:
                    self.isVirusResistant = False
##            print self.isVirusResistant
            if self.isVirusResistant:
                
                self.resistentVirusCounter += 1
##                print self.resistentVirusCounter
...
def problem4():
    ...
    for x in xrange(numberOfUpdates):
        testPerson.update()
        print testPerson.getTotalPop(), testPerson.getResistPop(['guttagonol'])
        virusPopList.append(testPerson.getTotalPop())
